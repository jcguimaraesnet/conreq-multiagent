...
Uses pdfplumber for table extraction and the user's configured LLM provider
for AI-powered requirement extraction from PDF documents.
"""
...
async def extract_requirements_with_ai(file_content: bytes, provider: LLMProvider = "gemini") -> ExtractedRequirements:
    """
    ...
    then uses the configured LLM provider to analyze and structure the requirements.

    Args:
        file_content: The PDF file content as bytes.
        provider: The LLM provider to use for AI extraction.
